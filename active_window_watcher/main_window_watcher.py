...
class ActiveWindowWatcher:
    ...
    def on_change_app(self, current_window, ):
        ...
        if self.active_log is not None:
            ...
            self.active_log.end = current
            self.active_log.duration = (current - self.active_log.start).total_seconds()
            self.conn.commit()
        ...
        self.active_log = create_and_insert_user_log(data={
            'user_id': self.user_id,
            'window_id': self.active_window.id,
            # TODO: from where the fuck to get this on
            # TODO: this might not work as it is totally different process
            'project_timeline_id': MainController.CURRENT_ACTIVE_PROJECT_TIMELINE_ID,
        }, session=self.conn)

    def on_new_app(self, current_window):
        print('New Application Dedicated')
        print(current_window)
