...
def get_window_name(window: Window) -> str:
    ...
    try:
        ...
        logger.warning(
            "Unable to get window property NET_WM_NAME, got a {} exception from Xlib".format(type(e).__name__))
    ...
    if d is None or d.format != 8:
        ...
        try:
            ...
            logger.warning(
                "Failed to decode one or more characters which will be skipped, bytes are: {}".format(d.value))
...
def get_window_class(window: Window) -> str:
    ...
    if not cls:
        ...
        try:
            ...
            logger.warning(
                "Unable to get window query_tree().parent, got a {} exception from Xlib".format(type(e).__name__))
            return "unknown"
        if window:
