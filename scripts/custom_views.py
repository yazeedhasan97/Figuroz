...
class CustomTimer(QWidget, ObservedTimer, metaclass=CustomTimerMeta):
    def __init__(self, timetracker, timer: QTimer = None, label: QLabel = None, icon: QLabel = None, ):
        ...
        self.__timetracker = timetracker

        self.__label.setText(str(self.__timetracker.duration))
    ...
    def __get_time(self):
        return self.__timetracker.duration

    def __set_time(self, time: timedelta, reset: bool = False):
        if isinstance(time, timedelta):
            if reset:
                self.__timetracker.duration = time
            else:
                self.__timetracker.duration += time
        else:
    ...
    def start(self):
        ...
        if self.__running_count == 1:
            self.notify({
                'project_id': self.__timetracker.project_id,
                'task_id': self.__timetracker.task_id,
                'user_id': self.__timetracker.user_id,
            })
