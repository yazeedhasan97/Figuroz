...
class MainController:
    ...
    MOUSE_MOVE_COUNT = 0
    SYSTEM = None
    CURRENT_ACTIVE_PROJECT_ID = -1
    CURRENT_ACTIVE_TASK_ID = -1
    CURRENT_ACTIVE_APPLICATION_ID = -1
    CURRENT_ACTIVE_URL_ID = -1
    CURRENT_ACTIVE_PROJECT_TIMELINE_ID = -1
    ...
    @classmethod
    def store_current_active_project_timeline_id(cls, id):
        MainController.CURRENT_ACTIVE_PROJECT_TIMELINE_ID = id
...
class TimerObserver(AbstractObserver):
    ...
    def create(self, ids: dict) -> None:
        ...
        from database.models import create_and_insert_project_time_line
        self._tracker = create_and_insert_project_time_line(
            project_id=ids['project_id'],
            task_id=ids['task_id'],
            user_id=ids['user_id'],
            session=MainController.DB_CONNECTION
        )
        MainController.store_current_active_project_timeline_id(
            id=self._tracker.id
        )
    ...
    def update(self, ) -> None:
        ...
        self._tracker.end = datetime.now()
        ...
        try:
            ...
            # MainController.DB_CONNECTION.query(User)
